...
passagers_t CreerPassager(int id, chaine dest){
    passagers_t p;
    p=(passagers_t)malloc(sizeof(passager_t));
...
passagers_t AvoirDernierElementList(passagers_t ListePassagers)
{
	while(ListePassagers->suivant != NULL)
	{
...
passager_t SupprimerPassagerListeAttenteEtLeRetourner(passagers_t *ListePassagers,int iDPassager){
    ...
    if ((*ListePassagers)->id==iDPassager)
    {
        ...
        PassagerARetourner.suivant=NULL;
        *ListePassagers=(*ListePassagers)->suivant;
    ...
    while ((SiListeVide(PassagerCourant)==0)&&(PassagerCourant->id!=iDPassager)){
        PassagerParent=PassagerCourant;
        PassagerCourant=PassagerParent->suivant;
    }

    if(SiListeVide(PassagerCourant)==1){
    ...
    PassagerARetourner.id=PassagerCourant->id;
    strcpy(PassagerARetourner.destination,PassagerCourant->destination);
    PassagerARetourner.suivant=NULL;
    ...
    for (int i=0;i<NumQuaie;i++)
    {
        ...
        printf("Prochain train,ID=%d, a destination de %s dans le quai %d\n",TrainPourDepart.id,TrainPourDepart.destination,Quaies[i].id);
            ...
            for(int j=0;j<(NumDestination+1);j++){
                if(strcmp(TrainPourDepart.destination,Destinations[j][0])==0){
                    TrainPourDepart.kmRestant=atoi(Destinations[j][1]);
            ...
            while(ParcourirPassager!=NULL){
                if((strcmp(ParcourirPassager->destination,TrainPourDepart.destination)==0)&&(SiTrainRemplis(TrainPourDepart)==0))
                {
